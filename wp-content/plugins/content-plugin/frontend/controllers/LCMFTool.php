...
class LCMFTool extends LCMfrontend{
    
    function detail() {
        $detail = $this->lcmf_model->get_tool_detail_by_id($this->id);
        $types = $this->lcmf_model->get_types();
        if($detail['status'] == TRUE){
            if($detail['num_rows'] > 0){
                $module = $detail['result']->module;
                $this->data['detail'] = $detail['result'];
                $this->data['types'] = $types['result'];
    ...
    function submit_item() {
        ...
        if($save_result['status'] == TRUE){
            ...
            if (strpos($lcm_refferer_url, 'newitemform') !== false) {
                $lcm_refferer_url = str_replace('newitemform=tool', '', $lcm_refferer_url);
