 ...
 * @return mixed|string
 */
function tve_prepare_global_variables_for_front( $css_string = '' ) {
	if ( is_editor_page() ) {
		return $css_string;
	}
...
function tve_render_widget_menu( $attributes ) {
	...
	remove_filter( 'wp_nav_menu_items', 'tve_menu_add_li_data_css' );
...
/**
 * Adds attributes to custom menu items
 *
 * @param $items - custom menu items
 * @param $args
 *
 * @return string
 */
function tve_menu_add_li_data_css( $items, $args ) {
	$dom = new DOMDocument();
	...
	foreach ( $find as $key => $item ) {
		if ( ! empty( $GLOBALS['tve_menu_group_edit'][ $key ] ) ) {
			$item->setAttribute( 'data-css', $GLOBALS['tve_menu_group_edit'][ $key ] ['css_id'] );

			if ( ! empty( $GLOBALS['tve_menu_group_edit'][ $key ]['class'] ) ) {
				$class = $item->getAttribute( 'class' );
				$class .= ' ' . $GLOBALS['tve_menu_group_edit'][ $key ]['class'];
