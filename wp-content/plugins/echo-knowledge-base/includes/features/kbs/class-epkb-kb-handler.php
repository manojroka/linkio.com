...
class EPKB_KB_Handler {
	...
	private static function create_sample_categories( $new_kb_id ) {
		...
		$sample_category_name = __( 'Articles', 'echo-knowledge-base' );
		$sample_article_title = __( 'Knowledge Base Article', 'echo-knowledge-base' );
		$sample_cat = self::create_sample_category_and_article( $new_kb_id, $kb_category_taxonomy_name, $sample_category_name, $sample_article_title, null );
		if ( empty($sample_cat) ) {
			return;
		}

		// second create sub-category with an article
		$sample_category_name = __( 'Other', 'echo-knowledge-base' );
		$sample_sub_cat = self::create_sample_category_and_article( $new_kb_id, $kb_category_taxonomy_name, $sample_category_name, $sample_article_title . ' #2', $sample_cat['category_id'] );
