...
<?php 
    $tools_bot = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}seo_tools_list WHERE status = 'Published' ORDER BY votes DESC", OBJECT ); // Might also try with OBJECT_K
    // ORDER BY name ASC
    // $tools = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}seo_tools_list WHERE status = 'Published' ORDER BY name ASC", OBJECT ); // Might also try with OBJECT_K
    // set_transient( 'tools', $tools, 10 * MINUTE_IN_SECONDS  );
    set_transient( 'tools_bot', $tools_bot, 10 * MINUTE_IN_SECONDS  );
    function fixUrl_b($url) {
        if( substr($url, 0, 3) === "www" )
                $url = 'http://' . $url;
        return $url;
    }
    foreach($tools_bot as &$tool) {
        $tool->homepage_url = fixUrl_b($tool->homepage_url);
